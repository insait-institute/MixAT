...
def display_result_single(args):
    if args.input_file is None:
        input_file = (
            f"data/{args.bench_name}/model_judgment/{args.judge_model}_single2_{args.model_list}.jsonl" 
        )
    else:
    ...
    if args.bench_name == "mt_bench":
        ...
        if df_2.shape[0] == 1:
            score_second =  df_2["score"].iloc[0]

        print("\n########## Average ##########")
        df_3 = df[["model", "score"]].groupby(["model"]).mean()
        print(df_3)
        score_avg = df_3["score"].iloc[0]
        print(df["model"].iloc[0])
        #assert df_3.shape[0] == 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--judge-model", type=str, default="gpt-4o") #gpt4 was default
